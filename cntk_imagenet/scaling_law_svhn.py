...
def main():
    ...
    # Set SVHN path
    svhn_path = None
    train_loader, test_loader = d.get_svhn_data(path=svhn_path)
    Xt_train, yt_train = d.normalized_numpy_data(train_loader, 10)
    Xt_test, yt_test = d.normalized_numpy_data(test_loader, 10)

    # Taking a subset of SVHN
    Xt_train = Xt_train[:5000]
    yt_train = yt_train[:5000]
    ...
    subsets = list(range(0, len(Xt_train), len(Xt_train)//50))[1:]
