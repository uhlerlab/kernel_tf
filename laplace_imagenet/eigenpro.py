...
class FKR_EigenPro(nn.Module):
    ...
    def __init__(self, kernel_fn, centers, y_dim, device="cuda", name='weights',
                 compute_eval=False):
        ...
        for epoch in epochs:
            ...
            if run_epoch_eval:
                ...
                if self.compute_eval:
                    ...
                    print("train error: %.5f\tval error: %.5f\ttrain acc: %.5f\tval acc: %.5f (%d epochs, %.2f seconds)\t"%(np.sqrt(tr_score['mse']), np.sqrt(tv_score['mse']), tr_score['multiclass-acc'], tv_score['multiclass-acc'], epoch, train_sec))
                    res.append(tv_score['multiclass-acc'])
                else:
                    print("train error: %.5f\ttrain acc: %.5f(%d epochs, %.2f seconds)\t"%(np.sqrt(tr_score['mse']), tr_score['multiclass-acc'], epoch, train_sec))
                    #res[epoch] = (tr_score['mse'])#, tv_score['mse'])
                    res.append(tr_score['mse'])
                #"""
            #"""
            initial_epoch = epoch
            pickle.dump(self.weight, open('saved_models/' + self.model_name + '.p', 'wb'), protocol=4)
        return np.array(res)
